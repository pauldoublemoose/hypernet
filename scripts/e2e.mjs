...
// ---------- Desktop keyboard flow (KNOWN COCREATOR) ------------------------
{
  ...
  await page.keyboard.press('Enter')
  await page.waitForSelector('button:has-text("TELL ME MORE ABOUT HYPERNET")')
  await page.screenshot({ path: `${SHOTS}/desktop-0-welcome.png` })
  await page.locator('button:has-text("TELL ME MORE ABOUT HYPERNET")').click()
  ...
  if ((await themeBtn.count()) !== 1) fail('Theme dock missing after onboarding')
  ...
  await themeBtn.click()
  if ((await appTheme()) !== 'white') fail('theme should return to white')
