...
async function driveDesktopChrome(page) {
  ...
  if (!(await page.locator('[data-shell="find"]').isVisible())) fail('Find control missing')
  ...
  const mine = page.locator('.shell-cat-toggle', { hasText: 'Mine' })
  if ((await mine.getAttribute('aria-expanded')) !== 'false') fail('Mine should start collapsed')
  await mine.click()
  if ((await mine.getAttribute('aria-expanded')) !== 'true') fail('Mine did not expand')
