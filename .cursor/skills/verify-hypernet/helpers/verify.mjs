...
async function driveDesktopChrome(page) {
  ...
  await page.locator('.title', { hasText: 'MH :: MY HORIZONS' }).waitFor({ state: 'visible' })
  await page.getByRole('button', { name: 'Groups', exact: true }).click()
  await page.locator('.title', { hasText: 'CL :: MY CLUSTERS' }).waitFor({ state: 'visible' })
  await page.getByRole('button', { name: 'Global Spam Hell', exact: true }).click()
  await page.getByRole('heading', { name: 'Global Spam Hell' }).waitFor({ state: 'visible' })
  await page.getByRole('button', { name: 'Moonwalker', exact: true }).click()
  await page.locator('.title', { hasText: 'MOONWALKER' }).waitFor({ state: 'visible' })
  await page.getByRole('button', { name: 'Bot Roulette', exact: true }).click()
  ...
  await page.getByRole('button', { name: 'Mystery Chat', exact: true }).click()
  await page.locator('[data-shell="mystery"]').waitFor({ state: 'visible' })
  await page.locator('.title', { hasText: 'MC :: MYSTERY CHAT' }).waitFor({ state: 'visible' })
  if ((await page.getByText('Storyphone').count()) > 0) fail('Storyphone string still in UI')
  await page.locator('[data-shell="mystery-join"]').click()
  await page.locator('[data-shell="mystery-live"]').waitFor({ state: 'visible' })
  const alias = (await page.locator('[data-shell="mystery-alias"]').innerText()).trim()
  if (!alias) fail('Mystery Chat alias missing')
  if (/anna|rio|kai|mira|jon|sasha/i.test(alias)) fail(`Mystery Chat leaked identity ${alias}`)
  const liveText = await page.locator('[data-shell="mystery-live"]').innerText()
  if (/anna vale|rio moss|kai okonkwo|mira chen|jon hale|sasha reed/i.test(liveText)) {
    fail('Mystery Chat leaked a directory name')
  }
  await page.locator('[data-shell="mystery-input"]').fill('hello from the booth')
  await page.locator('[data-shell="mystery-send"]').click()
  await page.locator('[data-shell="mystery-you"]').waitFor({ state: 'visible' })
  await page.screenshot({ path: `${EVIDENCE}/mystery-chat-live.png` })
  await page.locator('[data-shell="mystery-leave"]').click()
  await page.locator('[data-shell="mystery-ended"]').waitFor({ state: 'visible' })
  await page.screenshot({ path: `${EVIDENCE}/mystery-chat-ended.png` })
